...
function SellerSideBar() {
    ...
    const buttonStyle = {
        ...
        textAlign: 'center',
        padding: '10px',
        marginBottom: isOpen ? '10px' : '0', // Agregamos margen inferior solo cuando la barra está abierta
    };
        ...
        <div>
            ...
            <div style={isOpen ? sidebarStyle : { ...sidebarStyle, paddingTop: 0, paddingBottom: 0 }}> {/* Agregamos paddingTop y paddingBottom 0 cuando está cerrado */}
                {links.map((link, index) => (
                    <div key={index} className={`card mb-3 ${link.className ? 'border-danger' : 'border-success'}`} style={{ maxWidth: '18rem' }}>
